...
from Model.model_with_dilation import Lane_Detection_CPLUS
from Model.Fushion_model import Fushion_Lane_detection

import visdom
from tensorboardX import SummaryWriter
from Parameter import Parameters
from data_loader import Generator
from tools.loss import calculate_exist_loss,calculate_point_loss, calculate_point_chamfer_loss, lane_emd_loss, calculate_point_chamfer_loss_V2
from tools.MultiStepLR import MultiStepLR

##python -m visdom.server
p=Parameters()
writer = SummaryWriter('/home/doujian/Desktop/NCR_log2')
...
def Training():
    ...
    #model.load_state_dict(torch.load('/home/doujian/Desktop/NCR_LaneDetection/train_models/error_train_data_models/no_fushion/84_0.0894189178943634_SGD_lane_detection_network.pkl'))
    ...
    optim = torch.optim.RMSprop(parameters, lr=2.5e-4, weight_decay=weight_decay)
    #optim = torch.optim.SGD(model.parameters(), lr=0.01, momentum=0.9,weight_decay=weight_decay)
    #scheduler = CosineAnnealingLR(optim, T_max=138875, warmup='linear', warmup_iters=1000)
    scheduler = MultiStepLR(optim, iters_per_epoch=2777*40,warmup='linear', warmup_iters=1000)
##optimizer = optim.RMSprop(model.parameters(), lr=2.5e-4)
    ...
    for epoch in range(p.n_epoch):
        ...
        for raw_image, target_R2, target_R1, target_L2, target_L1, exist_label, test_image in loader.Generate():
            ...
            loss_point = calculate_point_chamfer_loss(R2Coors, target_R2, R2HeapMaps, R1Coors, target_R1, R1HeapMaps,
                                                      L2Coors, target_L2, L2HeapMaps, L1Coors, target_L1, L1HeapMaps,
                                                      exist_label)
            loss = loss_exist + loss_point

            writer.add_scalar('/home/doujian/Desktop/NCR_log2/loss_exist', loss_exist, step)
            #writer.add_scalar('/home/doujian/Desktop/NCR_log2/loss_prior', loss_prior, step)
            writer.add_scalar('/home/doujian/Desktop/NCR_log2/loss_point', loss_point, step)
            #writer.add_scalar('/home/doujian/Desktop/NCR_log2/inter_loss_point', inter_loss_point, step)
            writer.add_scalar('/home/doujian/Desktop/NCR_log2/loss', loss, step)
                    ...
                    if pred[0] == 1:
                        ...
                        for index in range(p.point_num):
                            image = cv2.circle(image, (int(tmp_L2Coors[index, 0]), int(tmp_L2Coors[index, 1])), 5,
                                               p.color[color_index], -1)
                    name = '/home/doujian/Desktop/NCR_log2/image_' + str(image_index)
...
def save_model(model, p, epoch, loss):
    torch.save(
            model.state_dict(),
            p.save_path2+str(epoch)+'_'+str(loss)+'_'+'SGD'+'_'+'lane_detection_network.pkl'
        )
