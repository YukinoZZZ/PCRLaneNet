...
def earth_mover_distance(xyz1, xyz2, transpose=True):
    ...
    if xyz1.shape[2]==2 and xyz2.shape[2]==2:
        zero_dim1 = torch.zeros((xyz1.shape[0],xyz1.shape[1],1)).cuda()
        zero_dim2 = torch.zeros((xyz2.shape[0], xyz2.shape[1], 1)).cuda()
        xyz1 = torch.cat((xyz1,zero_dim1),dim=2)
        xyz2 = torch.cat((xyz2,zero_dim2),dim=2)
