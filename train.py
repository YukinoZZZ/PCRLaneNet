...
from tools.loss import calculate_exist_loss,calculate_point_loss, calculate_point_chamfer_loss, lane_emd_loss,calculate_point_loss_with_lsq
...
def Training():
    ...
    model_path = '/data1/alpha-doujian/resnet101_models/' \
                 '12_0.6773213744163513_SGD_lane_detection_network.pkl'
    ...
    # model.load_state_dict(torch.load('/data1/alpha-doujian/save_model_wp/dilated_resnet34/90607.pkl'))
    ...
    optim = torch.optim.Adam(model.parameters(), lr=2.5e-4, weight_decay=weight_decay)
    #optim = torch.optim.SGD(model.parameters(), lr=1e-6, momentum=0.9,weight_decay=weight_decay)
    scheduler = MultiStepLR(optim, iters_per_epoch=5555*20)#, warmup='linear',warmup_iters=1000)
##optimizer = optim.RMSprop(model.parameters(), lr=2.5e-4)
    ...
    for epoch in range(p.n_epoch):
        ...
        for raw_image, target_R2, target_R1, target_L2, target_L1, exist_label, test_image in loader.Generate():
            ...
            writer.add_scalar('/home/doujian/Desktop/NCR_log2/midloss_point', midloss_point, step)
