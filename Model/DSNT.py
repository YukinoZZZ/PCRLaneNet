...
def smooth_l1_loss(actual, target, beta = 0.5):
    assert actual.size() == target.size(), 'input tensors must have the same size'

    diff = torch.abs(actual - target)
    diff_l1 = torch.zeros_like(diff).cuda()
    diff_l1[:, :, 0] = torch.where(diff[:, :, 0] < beta, 0.5 * diff[:, :, 0] * diff[:, :, 0] / beta,
                                diff[:, :, 0] - 0.5 * beta)
    diff_l1[:, :, 1] = torch.where(diff[:, :, 1] < beta, 0.5 * diff[:, :, 1] * diff[:, :, 1] / beta,
                                diff[:, :, 1] - 0.5 * beta)
    smoothL1loss = diff_l1.sum(-1,keepdim=False)

    return smoothL1loss

def l1_losses(actual,target):
    assert actual.size() == target.size(), 'input tensors must have the same size'
    return nn.functional.l1_loss(actual, target, reduction='none').mean(-1)
